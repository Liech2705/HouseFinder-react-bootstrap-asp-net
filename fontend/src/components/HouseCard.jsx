...
function HouseCard({ house, view }) { // Nhận thêm prop 'view' từ HouseList
    if (!house || house.status !== "visible" || !Array.isArray(house.rooms) || house.rooms.length === 0) {
        return null; // ❌ Không render gì nếu không có phòng
    }
    const houseData = house;
    const placeholder = 'https://surl.li/drynzt';
    // Booking statuses: { [roomId]: boolean }
    const [bookingStatuses, setBookingStatuses] = useState({});

    // Check booking status for each room when component mounts or rooms change
    useEffect(() => {
        if (!houseData.rooms || houseData.rooms.length === 0) return;
        setBookingStatuses({});
        houseData.rooms.forEach(async (room) => {
            try {
                const isBooked = await checkBooking(room.room_Id);
                setBookingStatuses(prev => ({ ...prev, [room.room_Id]: isBooked }));
            } catch (err) {
                console.error('Error checking booking for room', room.room_Id, err);
            }
        });
    }, [houseData.rooms]);
